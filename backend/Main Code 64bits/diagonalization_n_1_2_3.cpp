...
//    d[2].resize(nb_levels,nb_levels);
    ...
    const int nb_incoherent_low_levels=7; //  These are the "dead" levels number 0, 1, ... nb_incoherent_low_levels-1  for annihilation so for incoherent dipole sum
    const int nb_incoherent_high_levels=9; //  These are the "continuum" levels number nb_levels-nb_incoherent_high_levels, nb_levels-2, nb_levels-1  for photoionization so for incoherent dipole sum
